...
pub fn request_security_seed(
    ...
    if frame.data.len() != 5 || frame.data[0] != 0x01 {
        return Err(KernelError::UploadAborted(format!(
            ...
            frame.data.len(), frame.data.first(),
        )));
    }
...
pub fn submit_security_key(
    ...
    if frame.data.first().copied() != Some(0x02) {
        return Err(KernelError::UploadAborted(format!(
            "expected `67 02`, got data={:02X?}", frame.data,
        )));
    }
...
pub fn transfer_data_chunks(
    ...
    while offset < encrypted_buffer.len() {
        ...
        tracing::trace!(
            addr = format!("0x{addr:06X}"), chunk_len,
            "SID 0x36 chunk OK",
        );
...
pub fn upload_and_jump(
    ...
    let _ = pad_encrypt_and_upload(
        transport, trailer_addr, &CHECKSUM_BYPASS_TRAILER, timeout,
    )?;
...
mod tests {
    ...
    fn start_communication_sends_sid_81() {
        let mut mock = MockTransport::with_responses(vec![
            build_positive_response(sid::START_COMMUNICATION, &[0xEA, 0x8F]),
        ]);
    ...
    fn request_security_seed_returns_4_bytes() {
        ...
        let response = build_positive_response(sid::SECURITY_ACCESS, &[0x01, 0x12, 0x34, 0x56, 0x78]);
    ...
    fn unlock_full_flow_with_known_seed_key_pair() {
        ...
        let seed_response = build_positive_response(sid::SECURITY_ACCESS, &[0x01, 0x12, 0x34, 0x56, 0x78]);
