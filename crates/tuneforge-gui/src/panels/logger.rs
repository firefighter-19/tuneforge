...
use tracing::warn;
use tuneforge_defs::{parse_log_file, LoggerDocument, ResolvedLogEcu};
use tuneforge_io::serial::{SerialConfig, SerialTransport};
use tuneforge_io::Transport;
use tuneforge_logger::datalog::DatalogWriter;
#[cfg(feature = "ecu-tools")]
use tuneforge_logger::SampleValue;
use tuneforge_logger::{LoggerSession, Sample, SessionConfig};
...
pub enum LoggerMode {
    ...
    /// feature `ecu-tools` (тянет `tuneforge-kernel`, GPL-3.0).
    #[cfg(feature = "ecu-tools")]
    SsmCan,
}
...
impl LoggerPanel {
    ...
    fn start_kline_worker(&mut self) {
        ...
        let handle = std::thread::Builder::new()
            .name("tuneforge-logger-kline".into())
    ...
    fn start_ssm_can_worker(&mut self) {
        ...
        let handle = std::thread::Builder::new()
            .name("tuneforge-logger-ssm-can".into())
            .spawn(move || {
                ssm_can_worker(
                    tr,
                    chosen_raw,
...
#[cfg(feature = "ecu-tools")]
fn ssm_can_worker(
    mut tr: tuneforge_io::tactrix::TactrixTransport,
