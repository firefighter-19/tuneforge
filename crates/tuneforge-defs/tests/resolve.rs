...
use tuneforge_defs::{parse_file, resolve, ResolvedRom, StorageType, TableKind};
...
fn scalingbase_a2wc522s_target_boost_a_fully_resolved() {
    ...
    let has_bar = tb_a
        .scalings
        .iter()
        .any(|s| s.name.as_deref() == Some("BoostTarget_barabsolute"));
    let has_psi = tb_a
        .scalings
        .iter()
        .any(|s| s.name.as_deref() == Some("BoostTarget_psirelativesealevel"));
    assert!(has_bar && has_psi, "both scalings resolved by name");
    for s in &tb_a.scalings {
        assert!(
            s.expression.is_some(),
            "expression filled from scaling base"
        );
...
fn learning_airflow_a2wc510s_inherits_via_chain() {
    ...
    let t0 = leaf
        .tables
        .iter()
        .find(|t| t.name == "A/F Learning #1 Airflow Ranges")
        .unwrap();
...
fn x_axis_throttle_scaling_compiles_via_inline_definition() {
    ...
    let x_axis = tb_a
        .axes
        .iter()
        .find(|a| a.kind == Some(tuneforge_defs::TableKind::XAxis))
        .unwrap();
