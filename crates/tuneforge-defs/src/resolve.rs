...
impl<'a> Resolver<'a> {
    ...
    fn materialize_scaling(&self, raw: &ScalingRef) -> DefResult<ResolvedScaling> {
        ...
        if let Some(base_name) = raw.base.as_deref() {
            ...
            out.coarse_increment =
                parse_opt_f64(base.coarse_increment.as_deref(), "coarseincrement")?;
        ...
        if raw.category.is_some() {
            out.category = raw.category.clone();
        }
        if raw.units.is_some() {
            out.units = raw.units.clone();
        }
        if raw.expression.is_some() {
            out.expression = raw.expression.clone();
        }
        if raw.to_byte.is_some() {
            out.to_byte = raw.to_byte.clone();
        }
        if raw.format.is_some() {
            out.format = raw.format.clone();
        }
        if let Some(v) = parse_opt_f64(raw.fine_increment.as_deref(), "fineincrement")? {
            out.fine_increment = Some(v);
        }
        if let Some(v) = parse_opt_f64(raw.coarse_increment.as_deref(), "coarseincrement")? {
            out.coarse_increment = Some(v);
        }
        if let Some(v) = parse_opt_f64(raw.min.as_deref(), "min")? {
            out.min = Some(v);
        }
        if let Some(v) = parse_opt_f64(raw.max.as_deref(), "max")? {
            out.max = Some(v);
        }
...
fn merge_nested(parent: &[TableDef], child: &[TableDef]) -> Vec<TableDef> {
    ...
    for ch in child {
        let p_idx = result
            .iter()
            .position(|p| match (p.name.as_deref(), ch.name.as_deref()) {
...
mod tests {
    ...
    fn nested_axes_merged_by_kind_when_unnamed_in_child() {
        ...
        let x = t
            .axes
            .iter()
            .find(|a| a.kind == Some(TableKind::XAxis))
            .unwrap();
        ...
        let y = t
            .axes
            .iter()
            .find(|a| a.kind == Some(TableKind::YAxis))
            .unwrap();
