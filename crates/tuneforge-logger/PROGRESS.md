...
и движок, и плагины. Мы держим UI в `tuneforge-gui`, а здесь — только
...
Парсер `log_defs.xml` (наш `tuneforge-defs::logger`).

### `logger/ecu/profile/*`
Пользовательский профиль логгера (какие параметры выбраны, gauge layout).

### `logger/ecu/ui/*` — Swing UI логгера
Будет реализовано в `tuneforge-gui` (или вообще отложено — GUI-логгер ещё не начат).
...
- [ ] **`tuneforge-cli logger` команда** — headless логгирование в CSV без GUI
