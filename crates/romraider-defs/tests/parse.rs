...
fn scalingbase_test_fixture_has_expected_shape() {
    ...
    let base = doc.find_rom_by_xml_id("32BITBASE").expect("32BITBASE present");
    assert!(base.base.is_none(), "base ROM should not inherit");
    assert_eq!(base.tables.len(), 4, "base ROM tables");

    let child = doc.find_rom_by_xml_id("A2WC522S").expect("A2WC522S present");
...
fn scalingbase_test_target_boost_a_has_all_metadata() {
    ...
    assert!(tb_a.scalings.iter().any(|s| s.base.as_deref() == Some("BoostTarget_barabsolute")));
    assert!(tb_a.scalings.iter().any(|s| s.base.as_deref() == Some("BoostTarget_psirelativesealevel")));

    assert!(tb_a.description.as_deref().unwrap().contains("desired boost targets"));
}
