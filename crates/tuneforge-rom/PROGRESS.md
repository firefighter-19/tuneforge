...
- `Table1DView.java`, `Table2DView.java`, `Table3DView.java` — Swing-view (UI-layer; в нашем разделении это `tuneforge-gui`)
- `TableView.java` — общая база view
- `DataCell.java`, `DataCellView.java` — одна ячейка
- `Scale.java` — переехало в `tuneforge-defs::ResolvedScaling`
...
| Парсятся в `tuneforge-defs`, но не отображаются | inline в `Rom`               | —             |   ❌   |

### Прочее

| Фича                                          | Java-аналог            | Rust                        | Статус |
| --------------------------------------------- | ---------------------- | --------------------------- | :----: |
| `RomID` структура (xml_id + ecu_id + …)       | `RomID.java`           | (живёт в `tuneforge-defs::RomId`) | ✅ |
