...
use tuneforge_io::serial::{SerialConfig, SerialTransport};
use tuneforge_io::tactrix::{TactrixConfig, TactrixTransport};
use tuneforge_io::Transport;
use tuneforge_logger::{LoggerSession, Sample, SampleValue, SessionConfig};
use tuneforge_protocol::client::{probe, CanSsmClient, EcuClient, KLineSsmClient};
use tuneforge_protocol::ssm::{self, EcuInitResponse};
...
#[derive(Parser)]
#[command(name = "tuneforge", version, about = "tuneforge headless tools")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
fn logger_cmd(
    ...
    let doc = tuneforge_defs::parse_log_file(def_path)
    ...
    // 3. CSV-датлог.
    let mut datalog = tuneforge_logger::datalog::DatalogWriter::create(out_path)
        .with_context(|| format!("opening {}", out_path.display()))?;

    // 4. Открыть транспорт.
    let mut transport: Box<dyn tuneforge_io::transport::Transport> = if tactrix {
...
fn freeze_frame_can_cmd(timeout: Duration) -> Result<()> {
    use tuneforge_kernel::dtc_db::dtc_lookup;
    use tuneforge_kernel::orchestrator::read_freeze_frame;
    let mut tr = open_tactrix_can()?;
...
fn dtc_can_cmd(timeout: Duration) -> Result<()> {
    use tuneforge_kernel::dtc_db::dtc_lookup;
    use tuneforge_kernel::orchestrator::read_dtcs;
    let mut tr = open_tactrix_can()?;
...
fn tactrix_info_cmd() -> Result<()> {
    use tuneforge_io::tactrix::{find_tactrix, TACTRIX_OP2_PID, TACTRIX_VID};
    eprintln!(
...
fn inspect_def(
    ...
    let doc =
        tuneforge_defs::parse_file(path).with_context(|| format!("parsing {}", path.display()))?;
...
fn debug_kind(k: tuneforge_defs::TableKind) -> &'static str {
    use tuneforge_defs::TableKind::{
...
fn debug_storage(s: tuneforge_defs::StorageType) -> &'static str {
    use tuneforge_defs::StorageType::{
...
fn print_read_table(rom: &RomImage, table: &ResolvedTable) -> Result<()> {
    ...
    let endian = match table.endian {
        Some(tuneforge_core::Endian::Big) => "big",
        Some(tuneforge_core::Endian::Little) => "little",
        None => "?",
    };
    ...
    let scaling = table
        .scalings
        .first()
        .map(tuneforge_defs::ResolvedScaling::compile)
        .transpose()?;
...
fn print_grid(
    raw: &[f64],
    scaling: Option<&tuneforge_defs::CompiledScaling>,
    size_x: Option<u32>,
    units: &str,
) {
...
fn print_axis(
    ...
    let count = match axis.kind {
        Some(tuneforge_defs::TableKind::XAxis) => parent_size_x,
        Some(tuneforge_defs::TableKind::YAxis) => parent_size_y,
        _ => None,
    };
...
fn inspect_log(path: &PathBuf, ecu_filter: Option<&str>) -> Result<()> {
    let doc = tuneforge_defs::parse_log_file(path)
...
fn dump_rom_kernel_cmd(
    ...
    use tuneforge_kernel::{dump_rom_via_kernel, KernelDumpConfig, McuFamily};
...
fn dump_rom_can_cmd(
    ...
    use tuneforge_kernel::orchestrator::{dump_rom_via_can, DumpProgress};
...
fn peek_seed_cmd(send_key: bool, timeout: Duration) -> Result<()> {
    use tuneforge_kernel::orchestrator::uds_request;
    use tuneforge_kernel::seed_key::{subaru_genkey, subaru_genkey_can};
