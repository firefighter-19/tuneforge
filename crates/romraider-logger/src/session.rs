...
use romraider_defs::{CompiledLogParameter, StorageType};
...
impl LoggerSession {
    ...
    /// через [`romraider_defs::LogParameter::compile`].
...
/// копия `romraider_rom::decode_cells::decode_one` — логгер не должен
/// зависеть от `romraider-rom` (концептуально это другой домен).
...
mod tests {
    use super::*;
    use romraider_defs::{parse_log_str, LoggerDocument};
    ...
    fn pack_ssm_response(data_bytes: &[u8]) -> Vec<u8> {
        use romraider_protocol::ssm::{Command, ECU_ADDR, HEADER, TOOL_ADDR};
    ...
    fn poll_once_without_subscriptions_errors() {
        ...
        let mut transport = romraider_io::mock::MockTransport::new();
