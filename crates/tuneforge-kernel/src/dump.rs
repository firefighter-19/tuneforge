...
pub fn dump_rom_via_kernel(
    ...
    let init_response = tuneforge_protocol::ssm::ecu_init(transport, cfg.timeout)
...
mod tests {
    ...
    fn unaligned_start_rejected_early() {
        ...
        impl tuneforge_io::transport::Transport for DummyTransport {
            fn write_all(&mut self, _: &[u8], _: Duration) -> tuneforge_io::error::IoResult<()> {
                Ok(())
            }
            fn read_frame(
                &mut self,
                _: &mut [u8],
                _: Duration,
            ) -> tuneforge_io::error::IoResult<usize> {
                Ok(0)
            }
            fn purge(&mut self) -> tuneforge_io::error::IoResult<()> {
                Ok(())
            }
