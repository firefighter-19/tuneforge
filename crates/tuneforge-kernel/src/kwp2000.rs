...
pub fn parse_response(raw: &[u8]) -> KernelResult<(usize, Frame)> {
    if raw.len() < 6 {
        return Err(KernelError::Framing(format!(
            "frame too short: need ≥6 bytes, got {}",
            raw.len()
        )));
    }
    if raw[0] != HEADER {
        return Err(KernelError::Framing(format!(
            "expected header 0x80, got 0x{:02X}",
            raw[0]
        )));
    }
    let target = raw[1];
    ...
    if raw.len() < total_len {
        return Err(KernelError::Framing(format!(
            "frame truncated: need {total_len} bytes, got {}",
            raw.len()
        )));
    }
...
pub fn request_response(
    transport: &mut dyn tuneforge_io::transport::Transport,
    sid: u8,
    data: &[u8],
