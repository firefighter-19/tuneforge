...
`tuneforge-protocol` не знают, что под ними: реальный COM-порт, ELM327-донгл
или mock.
...
### `protocol/*` — переехало в **отдельный крейт `tuneforge-protocol`**
См. [`../tuneforge-protocol/PROGRESS.md`](../tuneforge-protocol/PROGRESS.md).
...
- [ ] **Slice 21: Kernel-upload-сценарий** — GPLv3 крейт `tuneforge-kernel`, через npkern/nisprog
